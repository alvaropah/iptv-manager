...
def main():
    ...
    # Workflows that start with a fresh workspace do not have the schema yet.
    # Initialize it here; if a persisted DB was restored, init_db is idempotent.
    init_db()
    with connect() as conn:
        ...
        for index, row in enumerate(rows, 1):
            try:
                result = client.episode(int(row["tmdb_series_id"]), int(row["season_number"]), int(row["episode_number"]))
                save_episode_metadata(conn, row, result)
                conn.commit()
                matched += 1
                print(f"[{index}/{len(rows)}] MATCHED | {row['provider_title']} -> {result.get('name') or '?'} | S{row['season_number']:02d}E{row['episode_number']:02d}", flush=True)
            except Exception as exc:
                errors += 1
                print(f"[{index}/{len(rows)}] ERROR | episode={row['episode_id']} | {exc}", flush=True)

        print(f"RESUMEN | matched={matched} errors={errors} pending={len(rows)-matched-errors}", flush=True)
