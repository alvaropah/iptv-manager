...
def save_episode_metadata(conn, row, result, matched_by="series_id+season_number+episode_number", source="tmdb", language=None):
    ...
    conn.execute("DELETE FROM metadata_links WHERE episode_id=? AND external_source=?", (row["episode_id"], source))
    conn.execute(
        ...
        (row["episode_id"], source, external_id, result.get("name"),
         result.get("overview") if source == "tmdb" else result.get("summary"),
         result.get("air_date") if source == "tmdb" else result.get("airdate"),
         result.get("runtime"), still_url, json.dumps(result, ensure_ascii=False),
         language or (settings.tmdb_language if source == "tmdb" else "en")),
    )
    conn.execute("DELETE FROM metadata_links WHERE external_source=? AND external_id=?", (source, external_id))
    conn.execute(
        ...
        VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP)""",
        (row["episode_id"], row["provider_title"], source, external_id, "matched", 1.0, matched_by),
    )
...
def resolve_series_tmdb_id(conn: object, client: TMDBClient, series_id: int, cache: dict[int, str | None], diagnostic: bool = False) -> tuple[str | None, bool]:
    ...
    existing = conn.execute("SELECT external_id FROM content_metadata WHERE content_id=? AND source='tmdb'", (series_id,)).fetchone()
    ...
    linked = conn.execute(
        """SELECT external_id FROM metadata_links
           ...
           ORDER BY updated_at DESC LIMIT 1""", (series_id,)
    ).fetchone()
    ...
    row = conn.execute(
        """SELECT id,content_type,canonical_title,original_title,year
           FROM content WHERE id=? AND content_type='series' AND is_active=1""", (series_id,)
    ).fetchone()
    if not row:
...
def _title_score(left: str, right: str) -> float:
    a = " ".join(_clean_title(left))
    b = " ".join(_clean_title(right))
    if not a or not b:
        return 0.0
    if a == b:
        return 1.0
...
def resolve_tvmaze_show_id(client: TVMazeClient, provider_title: str, cache: dict[str, int | None], diagnostic: bool = False) -> tuple[int | None, float]:
    """Find a high-confidence TVmaze show id once per provider title."""
    key = " ".join(_clean_title(provider_title))
    if key in cache:
    ...
    cache[key] = best_id
    if diagnostic:
        print(f"    TVMAZE SERIES {'MATCHED' if best_id else 'NO MATCH'} | query={query_used} | id={best_id or '?'} | score={best_score:.2f}", flush=True)
    return best_id, best_score


def resolve_episode_with_secondary(client: TVMazeClient, row, tvmaze_cache: dict[str, int | None], diagnostic: bool = False):
    ...
    print(f"    EPISODE SECONDARY | {row['provider_title']} | S{season:02d}E{episode:02d} | source=tvmaze | show={show_id} | score={score:.2f}", flush=True)
    return result, f"tvmaze+season_number+episode_number+score_{score:.2f}"


def resolve_episode_with_fallback(client: TMDBClient, row, primary_tmdb_series_id: str, tvmaze_client: TVMazeClient, tvmaze_cache: dict[str, int | None], diagnostic: bool = False):
        ...
        for score, candidate in ranked:
            ...
            try:
                ...
                if _http_status(exc) == 404:
                    continue
                raise
            print(f"    EPISODE FALLBACK | {provider_title} | S{season:02d}E{episode:02d} | primary={primary_tmdb_series_id} | fallback={candidate_id} | score={score:.2f} | query={query_used}", flush=True)
        ...
        for index, row in enumerate(rows, 1):
            try:
                ...
                if not tmdb_series_id:
                    ...
                    continue
                result, matched_by = resolve_episode_with_fallback(client, row, tmdb_series_id, tvmaze_client, tvmaze_cache, diagnostic=args.diagnostic)
                if result is None:
                    pending += 1
                ...
                save_episode_metadata(conn, row, result, matched_by=matched_by or "series_id+season_number+episode_number", source=source, language="en" if source == "tvmaze" else settings.tmdb_language)
                conn.commit()
                matched += 1
