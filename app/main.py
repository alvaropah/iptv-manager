...
APP_VERSION = "0.6.0"
app = FastAPI(title="IPTV Manager API", version=APP_VERSION, description="Biblioteca IPTV con metadatos persistentes. La navegación no consulta Xtream.")
...
@app.get("/")
def root() -> dict: return {"name": "IPTV Manager", "version": APP_VERSION, "status": "ok", "metadata": "tmdb-ready"}
@app.get("/api/health")
def health() -> dict: return {"status": "healthy", "version": APP_VERSION, "metadata": "tmdb-ready"}
@app.get("/api/stats")
def stats() -> dict: return get_stats()
@app.get("/api/categories")
def categories(content_type: str | None = Query(default=None, pattern="^(movie|series|live)$"), selected_only: bool = True) -> list[dict]: return get_categories(content_type=content_type, selected_only=selected_only)
