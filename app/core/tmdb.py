...
class TMDBClient:
    ...
    def _get(self, path: str, **params):
        r = requests.get(f"{BASE_URL}{path}", params={"language": self.language, **params}, headers={"Authorization": f"Bearer {self.token}", "accept": "application/json"}, timeout=30)
        r.raise_for_status()
        return r.json()

    def search_movie(self, title: str, year: int | None = None) -> list[dict]:
        data = self._get("/search/movie", query=title, primary_release_year=year) if year else self._get("/search/movie", query=title)
        return data.get("results", [])

    def search_tv(self, title: str, year: int | None = None) -> list[dict]:
        data = self._get("/search/tv", query=title, first_air_date_year=year) if year else self._get("/search/tv", query=title)
        return data.get("results", [])
